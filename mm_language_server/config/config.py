...
def add_args(
    parser: ArgumentParser, cfg: dict, prefix: str = ""
) -> ArgumentParser:
...
class Config:
    ...
    def _substitute_base_vars(
        cfg: Any, base_var_dict: dict, base_cfg: dict
    ) -> Any:
    ...
    def _file2dict(
        ...
        if DEPRECATION_KEY in cfg_dict:
            ...
            warning_msg = (
                f"The config file {filename} will be deprecated "
                "in the future."
            )
            if "expected" in deprecation_info:
                warning_msg += (
                    f' Please use {deprecation_info["expected"]} ' "instead."
                )
    ...
    @staticmethod
    def _merge_a_into_b(
        a: dict, b: dict, allow_list_keys: bool = False
    ) -> dict:
    ...
    def merge_from_dict(
        self, options: dict, allow_list_keys: bool = True
    ) -> None:
...
class DictAction(Action):
    ...
    def _parse_iterable(val: str) -> Union[list, tuple, Any]:
        ...
        def find_next_comma(string):
            ...
            All chars inside '()' and '[]' are treated as one element
            and thus ',' inside these brackets are ignored.
            """
