...
import parso
from parso.python.tree import Module, Name, PythonNode
...
def find_base_files(module: Module, root_path: Path) -> list:
    ...
    for definition in definitions:
        if definition.value == '_base_':
            base_node: ast.Assign = ast.parse(
                definition.parent.get_code()).body[0]
...
def search_def_in_bases(name, base_files):
    for base in base_files:
        ...
        defs = script._get_module_context().goto(name,
                                                 script._module_node.end_pos)
        if defs:
            defs = [
                classes.Name(script._inference_state, d) for d in set(defs)
            ]
            return list(set(helpers.sorted_definitions(defs)))
...
def get_type_defs(leaf: PythonNode, script: MMScript):
    ...
    registry = None
    if full_arg_name in ['model.type', 'model.backbone.type', 'model.neck.type', 'model.head.type']:
        __import__(default_scope + '.models')
        registry = __import__(default_scope + '.registry').registry.MODELS
    elif full_arg_name == 'model.init_cfg.type':
        registry = __import__(default_scope + '.registry').registry.WEIGHT_INITIALIZERS

    if registry is not None:
        class_ = registry.get(leaf.value.strip('\'\"'))
        pseudo_node = parso.parse(f'from {class_.__module__} import {class_.__name__}')
        classes_ = goto_import(script._get_module_context(),
